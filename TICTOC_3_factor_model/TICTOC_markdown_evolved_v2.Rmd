...
## 1. Read data  
When knitted, R Markdown runs from this file's own directory (`TICTOC_3_factor_model/`), so no
`setwd()` is needed. Run the analysis with this folder as the working directory. Portable check:
```{r, message=FALSE}
# Relative working directory — resolves to TICTOC_3_factor_model/ when knitting or when this
# repo folder is the R working directory. (Previously a hard-coded absolute path; de-hardcoded for reuse.)
...
> **Required inputs not committed to the repo** (obtain/regenerate before running; see
> `DATA_DICTIONARY.md` and `PROJECT_ROADMAP.md`): `Downloaded_Converted_Data.csv` (ID-converted
> expression matrix, derived from `TICTOC_run1_filteredCounts_v3.csv`), `iDEP_core_functions.R`,
> `Cotton__ghirsutum_eg_gene_GeneInfo.csv`, and `Cotton__ghirsutum_eg_gene.db` (iDEP species DB).
```{r, message=FALSE  }
